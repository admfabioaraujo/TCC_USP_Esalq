...
View(dataset_higienizado)
modelo_higienizado <- lm(dataset_higienizado$gdp_current_usd ~., data = dataset_higienizado)
summary(modelo_higienizado) #_ R Squared = 0.8869   R Adjusted = 0.8723

## Salvando o modelo, com base num dataset sem correlações acima de 0.8 e sem variáveis inflacionadas acimda de 10:
...
print(GDP_summarise)
library(e1071)
skewness(dataset_higienizado$gdp_current_usd) #_ 1.087796 - o coeficiente indica assimetria positiva, à direita, sendo a moda > média
kurtosis(dataset_higienizado$gdp_current_usd) #_ -0.1536578 - o coeficiente indica  alto grau de achatamento, a distribuição é platicúrtica
...
boxplot(dataset_higienizado_scaled, col="deepskyblue") -> boxplot_higienizado
...
title(main = "Gráfico Boxplot do dataset_higienizado - shape 62 X 8") -> boxplot_higienizado
...
library(nortest)
sf.test(modelo_higienizado$residuals) #_ p-value = 0.9251 > 0,05 - distribuição é normal
